...
<html>
  ...
  app.get('/auth/oidc/login', async (request, reply) => {
    if (!oidcConfig) {
      throw new ApiError(501, ErrorCodes.AUTH_NOT_CONFIGURED, 'OIDC is not configured on this server');
    }
  ...
  app.get('/auth/oidc/callback', async (request, reply) => {
    ...
    if (!state || !consumeOidcState(state)) {
      throw new ApiError(400, ErrorCodes.VALIDATION_ERROR, 'Invalid or expired OIDC state parameter');
    }
