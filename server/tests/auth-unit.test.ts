...
describe('auth (unit)', () => {
  ...
  describe('verifyRefreshToken error handling', () => {
    ...
    it('rejects an access token used as refresh token (wrong claims)', () => {
      // Access tokens have a 'role' claim but both use same issuer/audience,
      // so an access token would technically verify.  This test documents
      // that verifyRefreshToken does not reject access tokens by itself
      // (the jti check in rotateRefreshToken handles that).
      const user = makeUser();
      const accessToken = signAccessToken(user);
      // This should NOT throw because the JWT is structurally valid
      const payload = verifyRefreshToken(accessToken);
      expect(payload.sub).toBe(user.id);
      // But it won't have a jti, which rotateRefreshToken checks
