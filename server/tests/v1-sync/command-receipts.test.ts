...
import { createEntryCommand, executeSyncCommands, installV1SyncSuite } from './support.js';
...
describe('v1 sync command receipts and versions', () => {
  ...
  it('returns a duplicate receipt without applying the create twice', async () => {
    ...
    const equivalent = await execute(suite.studentToken, [create('v1-create-equivalent')]);
    ...
    const conflict = await execute(suite.studentToken, [conflicting]);
