...
final class APIClientRequestEncodingTests: XCTestCase {
    ...
    private func makeTestArtifact(id: String = "artifact-1") -> LocalArtifact {
        LocalArtifact(
            id: id,
            entryId: "entry-1",
            type: .audio,
    ...
    func testCreateArtifactEncodesLocalFileByteSize() async throws {
        ...
        let capture = installRequestBodyCapture(
            expectedURL: expectedURL,
            method: "POST",
            response: Data("{\"id\":\"artifact-1\",\"entryId\":\"entry-1\",\"type\":\"audio\",\"durationSeconds\":30,\"uploadState\":\"pending\",\"storageKey\":null,\"remoteUrl\":null}".utf8)
        )

        _ = try await makeCapturingAPIClient().createArtifact(
            accessToken: "access-token",
            entryId: "entry-1",
